import { Edit2, Trash2, User } from 'lucide-react'
import { Card, CardHeader, CardTitle, CardDescription, CardContent } from '#/components/ui/card'
import { Button } from '#/components/ui/button'
import { formatPhone } from '#/lib/format'
import type { Professional } from '../types'

export type ProfessionalCardProps = {
  professional: Professional
  onEdit: (professional: Professional) => void
  onDelete: (professional: Professional) => void
}

export function ProfessionalCard({ professional, onEdit, onDelete }: ProfessionalCardProps) {
  return (
    <Card className="flex flex-col relative overflow-hidden group">
      <CardHeader className="pb-3 flex-row items-start justify-between">
        <div className="flex gap-3 items-center">
          <div className="h-10 w-10 rounded-full bg-zinc-900 flex items-center justify-center shrink-0 border border-zinc-800">
            <User className="w-5 h-5 text-zinc-400" />
          </div>
          <div>
            <CardTitle className="text-base text-zinc-100">{professional.name}</CardTitle>
            {(professional.email || professional.phone) && (
              <CardDescription className="flex flex-col mt-0.5 text-xs text-zinc-400 gap-0.5">
                {professional.email && <span>{professional.email}</span>}
                {professional.phone && <span>{formatPhone(professional.phone)}</span>}
              </CardDescription>
            )}
          </div>
        </div>
      </CardHeader>
      
      <CardContent className="flex-1 flex flex-col justify-end pt-2">
        <div className="flex items-center justify-between mt-auto">
          <div className="flex items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity ml-auto">
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800"
              onClick={() => onEdit(professional)}
              aria-label="Editar"
            >
              <Edit2 className="w-4 h-4" />
            </Button>
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 text-rose-400 hover:text-rose-300 hover:bg-rose-500/10"
              onClick={() => onDelete(professional)}
              aria-label="Excluir"
            >
              <Trash2 className="w-4 h-4" />
            </Button>
          </div>
        </div>
      </CardContent>
    </Card>
  )
}
