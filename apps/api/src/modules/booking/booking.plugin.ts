...
export function createBookingModulePlugin(notificationsService: NotificationsService): FastifyPluginCallback {
  return (fastify, _opts, done): void => {
    ...
    fastify.post(
      ...
      async (request, reply) => {
        ...
        await notificationsService.scheduleAfterBooking(result.id);
    ...
    fastify.post(
      ...
      async (request, reply) => {
        ...
        await notificationsService.onClientCancelledAppointment(result.appointmentId);
